...
CASE 
  WHEN p.price <= 50000 THEN p.price * (1 - t.discount_percentage) * 0.1
  WHEN p.price <= 100000 THEN p.price * (1 - t.discount_percentage) * 0.15
  WHEN p.price <= 300000 THEN p.price * (1 - t.discount_percentage) * 0.2
  WHEN p.price <= 500000 THEN p.price * (1 - t.discount_percentage) * 0.25
  ELSE p.price * (1 - t.discount_percentage) * 0.3
END AS nett_profit,
